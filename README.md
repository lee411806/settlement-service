...
- **Replica를 활용해 생성 DB와 조회 DB를 분리**, 읽기 요청을 분산하여 처리 부하 완화  
- **Batch로 생성된 대량 데이터를 일괄 처리해 조회 전용 테이블에 적재**, 대량 조회 시 RDBMS 부하 최소화  
- **Redis를 적용해 실시간 조회 데이터를 초저지연으로 제공**, 응답 속도와 사용자 경험 개선  
- **JMeter 부하 테스트를 활용해 병목 구간 진단 및 개선 적용**  

<br>

## 결과

1. **Batch 활용 3,200만 건 기준 2분 12초 처리 (99.83% 성능 향상)**  
2. **조회 API JMeter 기준 TPS 18.3배 증가, P99 응답 속도 1,634배 개선**  
    - *테스트 종료 직후 POST 요청의 지연으로 P99 수치가 일시적으로 튀었지만, 정상 구간 기준으로는 안정적인 개선 확인*  
3. **Redis 기반 100만 건 기준 어뷰징 방지 로직: 283ms → 14ms (95.05% 향상)**  
