...
<sub> 1. JMeter 1분 duration 설정 시, 1분 시점까지는 성능이 양호해 보였으나 실제로는 병목으로 인해 요청이 큐에 누적된 상태였음. 테스트 종료 시점에 누적된 요청들이 일괄 처리되면서 성능이 급격히 악화됨을 확인.</sub> <br>
<sub>2. 당시 조회 성능 개선에만 집중하여, 개선 전 측정에서 심각한 병목 상황을 "적용 전" 기준으로 설정함. 이로 인해 1634배라는 비현실적인 개선 수치가 도출됨. 실제로는 개선 전 상태가 정상적인 성능 기준이 아닌 극심한 병목 상태였으며, 
개선 후의 수치도 병목 해소로 인한 정상화에 가까웠을 것으로 분석됨. </sub> <br>
<sub>3. **해당 성능 병목 이슈는 추후 MSA 전환 프로젝트에서 근본적으로 해결됨.** </sub>
